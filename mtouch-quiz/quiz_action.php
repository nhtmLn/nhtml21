...
$single_page = '0';
$quiz_category = mtq_array_element('quiz_category', $_POST);
$quiz_difficulty = mtq_array_element('quiz_difficulty', $_POST);
...
if(isset($_REQUEST['submit'])) {
	if($_REQUEST['action'] == 'edit') { //Update goes here
		...
		$wpdb->get_results($wpdb->prepare("UPDATE {$wpdb->prefix}mtouchquiz_quiz SET name=%s, description=%s,final_screen=%s,answer_mode=%s,single_page=%s, show_hints=%s, show_start=%s, show_final=%s, multiple_chances=%s, random_questions=%s, random_answers=%s, form_code=%s, time_limit=%s, quiz_category=%s, quiz_difficulty=%s WHERE ID=%d", sanitize_text_field($_REQUEST['name']), $_REQUEST['description'], $_REQUEST['content'], $answer_mode, $single_page, $show_hints, $show_start, $show_final,$multiple_chances, $random_questions, $random_answers,$_REQUEST['gravity'],$_REQUEST['mtq_timer'], $quiz_category, $quiz_difficulty, $_REQUEST['quiz']));
	...
	foreach ($_REQUEST['score_rating'] as $score_rating) {
		...
		$wpdb->get_results($wpdb->prepare("INSERT INTO {$wpdb->prefix}mtouchquiz_quiz(name,description,final_screen, answer_mode, single_page, show_hints, show_start, show_final, multiple_chances, random_questions, random_answers, form_code, quiz_category, quiz_difficulty, time_limit, added_on) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,NOW())", $_REQUEST['name'], $_REQUEST['description'], $_REQUEST['content'], $answer_mode, $single_page, $show_hints, $show_start, $show_final,$multiple_chances, $random_questions, $random_answers,$_REQUEST['gravity'],$quiz_category,$quiz_difficulty,$_REQUEST['mtq_timer']));
