<?php
	...
	if (true) {
		...
		$quiz_options = $wpdb->get_row($wpdb->prepare("SELECT name,description,answer_mode,single_page,show_hints,show_start,show_final,multiple_chances,final_screen,random_questions,random_answers, quiz_difficulty, quiz_katlink FROM {$wpdb->prefix}mtouchquiz_quiz WHERE ID=%d", $quiz_id));
		...
		if( $random_questions == 1 && $input_number_questions > 0 ) { // Select questions randomly
			$all_question = $wpdb->get_results($wpdb->prepare("SELECT ID,question,explanation, point_value, soru_tipi FROM {$wpdb->prefix}mtouchquiz_question WHERE quiz_id=%d AND ID>=$first_id_value AND ID<=$last_id_value ORDER BY RAND() LIMIT 0, $input_number_questions", $quiz_id));
		} elseif( $random_questions != 1 && $input_number_questions > 0 ) { // Select questions in order
			$all_question = $wpdb->get_results($wpdb->prepare("SELECT ID,question,explanation, point_value, soru_tipi FROM {$wpdb->prefix}mtouchquiz_question WHERE quiz_id=%d AND ID>=$first_id_value  AND ID<=$last_id_value ORDER BY ID LIMIT 0, $input_number_questions", $quiz_id)); // Not random
		} else { // select all questions in order
			$all_question = $wpdb->get_results($wpdb->prepare("SELECT ID,question,explanation, point_value, soru_tipi FROM {$wpdb->prefix}mtouchquiz_question WHERE quiz_id=%d ORDER BY ID", $quiz_id)); // Not random, get everything, fallback else
		}
...
<?php
            ...
            $question_count = count($all_question);
            $time_human = $mtq_max_time / 60;
			?>
<?php 
...
<div id="mtq_quiz_results_bubble-<?php echo $mtqid ?>" class="mtq_quiz_results_bubble"> <div id="mtq_quiz_results-<?php echo $mtqid ?>" class="mtq_quiz_results"><?php echo str_replace('%%QUIZ_NAME%%','<em>'.stripslashes($quiz_options->name).'</em>',$final_screen);?> <br>    
    ...
    <div id="mtq_contact_form-<?php echo $mtqid ?>"> <?php echo nl2br($form_code); ?> </div>
  ...
  <div <?php if (!$single_page) { echo "class='mtqscrollable' id='mtq_scroll_container-{$mtqid}'";}?>>
    ...
    <div id="mtq_scroll_items_container-<?php echo $mtqid ?>" class="items">
      ...
      foreach ($all_question as $ques) {
	      ...
	      echo   "<table class='mtq_question_heading_table tablo'><tr><td>";
	      if ( $show_labels ) {
	      ...
	      foreach ($dans as $ans) {
		      ...
		      echo   "<tr id='mtq_row-{$question_count}-{$answer_count}-$mtqid' onclick='mtq_button_click({$question_count},{$answer_count},$mtqid)' class='mtq_clickable {$tip_soru}'>";
		      ...
		      echo   "<div id='mtq_answer_text-{$question_count}-{$answer_count}-$mtqid' class='mtq_answer_text {$cevap_bosluk}'>".stripslashes(trim($ans->answer))."</div>";
		      $is_correct_value = '0';
