...
$question= $wpdb->get_row($wpdb->prepare("SELECT question, explanation, point_value, soru_tipi FROM {$wpdb->prefix}mtouchquiz_question WHERE ID=%d", $_REQUEST['question']));
...
<div class="wrap">
    ...
    <form name="post" action="edit.php?page=mtouch-quiz/question.php&amp;quiz=<?php echo $_REQUEST['quiz']; ?>" method="post" id="post">
        <div id="poststuff">
            ...
            Soru Tipi
                    ...
                    <div class="inside">
						...
						if($action == 'new'){
							?>
                            <select name="soru_tipi">
                                <option value="1">Resimli Soru</option>
                                <option value="2">Yazılı Soru</option>
                            </select>
							<?php
						}else{
							if($question->soru_tipi == '1'){
								?>
                                <select name="soru_tipi">
                                    <option value="1" selected>Resimli Soru</option>
                                    <option value="2">Yazılı Soru</option>
                                </select>
								<?php
							}else{
                                ...
                                <select name="soru_tipi">
                                    <option value="1">Resimli Soru</option>
                                    <option value="2" selected>Yazılı Soru</option>
                                </select>
								<?php
							}
						}
		...
		<?php wp_nonce_field('mtq_question_form'); ?>
    </form>
</div>
