...
if($action == 'edit') {
	...
	$dquiz = $wpdb->get_row($wpdb->prepare("SELECT name,description,final_screen,answer_mode,single_page,show_hints,show_start, show_final,multiple_chances,random_questions,random_answers, quiz_category, quiz_difficulty, quiz_katlink FROM {$wpdb->prefix}mtouchquiz_quiz WHERE ID=%d", $clean_quiz));
	$quiz_category = stripslashes($dquiz->quiz_category);
	$quiz_difficulty = stripslashes($dquiz->quiz_difficulty);
	...
	$dquizfm = $wpdb->get_row($wpdb->prepare("SELECT form_code FROM {$wpdb->prefix}mtouchquiz_quiz WHERE ID=%d", $clean_quiz));
	...
	$final_screen = __("<p style=\"text-align: left;\"><strong>Tebrikler!</strong> <strong>%%QUIZ_NAME%%</strong> adlı sınavı başarıyla tamamladınız.</p>
<p style=\"text-align: left;\">→ <strong>%%TIME_ALLOWED%%</strong> saniye süre verilen sınavı <strong>%%TIME_USED%%</strong> saniyede tamamladınız.</p>
<p style=\"text-align: left;\">→ Toplam soru sayısı:<strong> %%TOTAL%%</strong></p>
<p style=\"text-align: left;\">→ Doğru sayınız: <strong>%%SCORE%%</strong> - yanlış ve boş sayınız: <strong>%%WRONG_ANSWERS%%.</strong></p>
<p style=\"text-align: left;\">→ Sınavdan aldığınız puan: <strong>%%PERCENTAGE%%</strong></p>
<p style=\"text-align: left;\">→ Sınav sonucu değerlendirmeniz:</p>
<p style=\"text-align: left;\">→ <strong>%%RATING%%</strong></p>", 'mtouchquiz');
}
...
?>
  <form name="post" action="<?php echo $GLOBALS['wpframe_plugin_folder'] ?>/quiz_action.php" method="post" id="post">
    <div id="poststuff">
        ...
        <div class="postbox" id="titlediv">
            <h3 class="hndle"> <span>
          <?php _e('Difficulty', 'mtouchquiz') ?>
          </span> </h3>
            <!--p align="right"> <a class="button toggleVisualtwo"><?php _e('Visual', 'mtouchquiz') ?></a> <a class="button toggleHTMLtwo"><?php _e('HTML', 'mtouchquiz') ?></a> </p-->
            <div class="inside">
                <input type="text" name="quiz_difficulty" value="<?php echo $dquiz->quiz_difficulty ? stripslashes($dquiz->quiz_difficulty) : 'Orta'; ?>"/>
            </div>
        </div>
        ...
        <div class="postbox" id="titlediv">
          ...
          <?php _e('Category') ?>
            ...
            <div class="inside" style="width: 100%">
                <select name="quiz_category" style="width: 100%">
				    <?php if($categories){
					    foreach($categories as $category) {
						    echo $quiz_category;
						    $selected = $category->term_id == $quiz_category ? 'selected' : '';
      ...
      <div class="postbox">
        ...
        <div class="inside">
          ...
          <?php
			...
			$default_messages = array(__("Daha fazla konu çalışmalı ve sonrasında testlere ağırlık vermelisiniz!", 'mtouchquiz'),__("Böyle devam et, daha yapacak çok test var!", 'mtouchquiz'),__("Hiç fena değil, testlere devam!", 'mtouchquiz'),__("Güzel sonuç, böyle devam!", 'mtouchquiz'),__("Harikasın!", 'mtouchquiz'));
			$num_ratings = 5;
      ...
      <div class="postbox">
        ...
        <div class="inside">
          ...
          <thead>
            ...
            <tr>
              <td><input type="checkbox" name="multiple_chances" <?php if($multiple_chances == '2') echo 'checked="checked"'; else if ($action == 'new') { echo 'checked="checked"';} ?> value="2" id="multiple_chances" />
	...
	if ( ! $mtq_timer_allgood ) { ?>
      ...
      <?php } else {
		   ...
		   ?></h4>
		  <textarea name="mtq_timer" rows="1" cols="100"><?php echo $mtq_time ? $mtq_time : 120 ?></textarea>
	<?php  } ?>
