...
if(isset($_REQUEST['submit']) && check_admin_referer('mtq_question_form') ) {
	...
	$soru_tipi = intval($_REQUEST['soru_tipi']);
	if($action == 'edit'){ //Update goes here //, $_REQUEST['question']
		$wpdb->query($wpdb->prepare("UPDATE {$wpdb->prefix}mtouchquiz_question SET question=%s, explanation=%s, number_correct=%d, point_value=%d, soru_tipi=%d WHERE ID=%d", $_REQUEST['content'], $_REQUEST['explanation'], $num_correct,$point_value,$soru_tipi,$_REQUEST['question'] ));
		...
		$wpdb->query($wpdb->prepare("INSERT INTO {$wpdb->prefix}mtouchquiz_question(quiz_id, question, explanation,number_correct, point_value, soru_tipi) VALUES(%d, %s, %s, %d, %d, %d)", $_REQUEST['quiz'], $_REQUEST['content'], $_REQUEST['explanation'],  $num_correct, $point_value, $soru_tipi));//Inserting the questions
	...
	foreach ($_REQUEST['answer'] as $answer_text) {
		...
		if(!$answer_text || $answer_text || is_numeric($answer_text)) {
