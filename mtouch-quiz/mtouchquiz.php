...
define( 'mtq_database_version','1.6.5.8');
...
';
    if ($_POST['mtq_hidden'] == 'Y') {
        if (check_admin_referer('mtq_options_form')) {
			//process form
...
<form id="mtouchquiz" name="mtouchquiz" action="" method='POST'>
  ...
  <?php wp_nonce_field('mtq_options_form'); ?>
</form>
<br />
	...
	if($database_version != $installed_db) {
		...
		$sql = "CREATE TABLE {$wpdb->prefix}mtouchquiz_answer (
				...
				CREATE TABLE {$wpdb->prefix}mtouchquiz_quiz (
					...
					katlink varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
					...
					quiz_category int(11) unsigned NOT NULL,
					quiz_difficulty varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
					quiz_katlink varchar(100) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
					PRIMARY KEY  (ID)
				);";
		dbDelta($sql);
...
function mtq_array_element($element, $array, $default = null)
{
	$element = !empty($array[$element]) ? $array[$element] : $default;
	$element = is_array($element) ? $element : trim($element);
